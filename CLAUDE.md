...
`GET /api/user/me` and `GET /api/students/me` record one `user_activities` row per authenticated user per UTC calendar day; `POST /api/user/me/activity` records it explicitly (any role) and returns `{ activityDate, recorded }`, where `recorded` is `false` if today's row already existed. A unique constraint on `(user, activityDate)` plus `ON CONFLICT DO NOTHING` makes repeated calls—across any of these endpoints—idempotent. Admin `GET /api/stats/summary` includes DAU (today), WAU (today plus the previous 6 days), and MAU (today plus the previous 29 days). `GET /api/stats/series` and the backwards-compatible `/timeseries` separate `businessMetrics` (new users/assignments/enrollments/mentors for the requested 7, 14, or 30 days) from `activeUserMetrics`: DAU has one point per elapsed day of the current UTC month, WAU uses consecutive seven-day buckets within the current month, and MAU has one point for each of the latest six calendar months.
