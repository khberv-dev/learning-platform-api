...
**Sequential unlocking is on.** `withLessonsCount` (`course.service.ts`) marks a lesson locked when the **previous lesson in the same unit** has tasks and its progress is under `LESSON_UNLOCK_PERCENT` (80); `previousLessonId` resets at the start of every unit, so a unit's first lesson is always unlocked regardless of how the previous unit ended, and a previous lesson with no tasks never blocks either — otherwise a video-only lesson would be an impassable wall.
